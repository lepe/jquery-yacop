        ...
        this.each(function() {
			...
			if(options == "hide") {
				...
				if(cots) for(var c = 0; c < cots.length; c++) {
					cots[c].fadeOut(cots[c].data("fadeOut"));
				}
			} else if(options == "show") {
				var cots = caller.callout;
				if(cots) for(var c = 0; c < cots.length; c++) {
					cots[c].fadeIn(cots[c].data("fadeIn"));
				}
                ...
                $co.data("fadeout", options.fadeOut);
                $co.data("fadein", options.fadeIn);
				...
				$co.css("left",left);
				$co.css("top",top);
				if(options.show) $co.fadeIn(options.fadeIn);
